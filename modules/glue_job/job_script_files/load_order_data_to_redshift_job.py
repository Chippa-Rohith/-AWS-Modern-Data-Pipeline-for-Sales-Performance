...
GLUE_DATABASE = args['silver_database']
SILVER_TABLE = args.get('silver_table_name', 'Orders')
...
TARGET_TABLE = args.get('target_table', 'fact_orders')

print(f"Starting job to load orders data from {GLUE_DATABASE}.{SILVER_TABLE} "
      f"to Redshift {TARGET_SCHEMA}.{TARGET_TABLE}")

try:
    # Read silver layer orders data directly from S3
    print(f"Reading data from S3 path: s3://{S3_BUCKET}/silver-data/salesdb/Orders/")

    silver_orders_df = glueContext.create_dynamic_frame.from_options(
        connection_type="s3",
        format="parquet",
        connection_options={"paths": [f"s3://{S3_BUCKET}/silver-data/salesdb/Orders/"]},
        transformation_ctx="read_silver_orders"
    )

    print(f"Records read from silver layer: {silver_orders_df.count()}")
    ...
    print(f"Successfully loaded {silver_orders_df.count()} records to Redshift")
