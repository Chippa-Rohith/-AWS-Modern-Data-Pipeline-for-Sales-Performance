...
TARGET_TABLE = args.get('target_table', 'stage_dim_product')

print(f"Starting job to load product data from {SILVER_DATABASE}.{SILVER_TABLE} "
      f"to Redshift {TARGET_SCHEMA}.{TARGET_TABLE}")

try:
    # Read silver layer product data directly from S3
    print(f"Reading data from S3 path: s3://{S3_BUCKET}/silver-data/salesdb/Product/")

    silver_product_df = glueContext.create_dynamic_frame.from_options(
        connection_type="s3",
        format="parquet",
        connection_options={"paths": [f"s3://{S3_BUCKET}/silver-data/salesdb/Product/"]},
        transformation_ctx="read_silver_product"
    )

    print(f"Records read from silver layer: {silver_product_df.count()}")
    ...
    print(f"Successfully loaded {silver_product_df.count()} records to Redshift")
